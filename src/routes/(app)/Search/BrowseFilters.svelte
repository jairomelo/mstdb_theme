<script>
	...
	$: {
		...
		for (const g of filterLayout.groups) {
			ng[g.name] =
				expandedGroups[g.name] !== undefined ? expandedGroups[g.name] : g.name === 'Trayectorias';
		}
		expandedGroups = ng;
	}
				...
				<div class="mb-3">
					...
					<div class="d-flex align-items-center gap-2">
						<input
							...
							on:keydown={(e) => {
								if (e.key === 'Enter') applyYearRange(filter.key, nextFilter.key);
							}}
						/>
		...
		{#each filterLayout.groups as group}
			...
			<div class="filter-group">
				<button class="filter-group-header" on:click={() => toggleGroup(group.name)}>
					<i class="bi {expandedGroups[group.name] ? 'bi-chevron-down' : 'bi-chevron-right'} me-1"
					></i>
				...
				{#if expandedGroups[group.name]}
					<div class="filter-group-body">
						{#each group.filters as filter, i}
							...
							{@const isRangePair =
								filter.type === 'number' &&
								nextFilter?.type === 'number' &&
								filter.key.replace(/__(gte|lte)$/, '') ===
									nextFilter.key.replace(/__(gte|lte)$/, '')}
								...
								<div class="mb-2">
									<label class="form-label small mb-1"
										>{filter.key
											.replace(/__(gte|lte)$/, '')
											.replace(/_/g, ' ')
											.replace(/^\w/, (c) => c.toUpperCase())}</label
									>
							...
							{:else if filter.type === 'number' && i > 0 && group.filters[i - 1]?.type === 'number' && filter.key.replace(/__(gte|lte)$/, '') === group.filters[i - 1].key.replace(/__(gte|lte)$/, '')}
