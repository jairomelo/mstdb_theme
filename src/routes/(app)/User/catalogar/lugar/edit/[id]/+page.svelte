<script>
	...
	function populateForm() {
		...
		tipo = lugar.tipo_id != null ? { value: lugar.tipo_id, label: lugar.tipo ?? '' } : null;
		es_parte_de =
			lugar.es_parte_de != null
	...
	function placeMarker(latV, lonV) {
		...
		marker = L.circleMarker([latV, lonV], {
			...
			.addTo(map)
			.bindPopup(nombre_lugar || 'Nuevo punto');
	...
	$: if (marker) {
		try {
			marker.setPopupContent(nombre_lugar || 'Nuevo punto');
		} catch {}
	}
					...
					<div class="card-body">
						<form on:submit|preventDefault={handleSave} novalidate>
							...
							<div class="row g-2 mb-3">
								<div class="col-6">
									<FormField label="Latitud" id="lat" error={fieldError('lat')} hint="Ej. 4.710989">
										<input
											id="lat"
											type="number"
											step="any"
							...
							<div class="d-flex gap-2 mt-3">
								...
								<a href="/Detail/lugar/{data.id}" class="btn btn-outline-secondary"> Cancelar </a>
							</div>
						</form>
					</div>
				</div>
								...
								<li class="list-group-item py-2 px-3">
									{#if mergeTarget === c}
										...
										<div class="d-flex flex-column gap-2">
											...
											{#if mergeError}
												<div class="alert alert-danger py-1 mb-0 small" role="alert">
													{mergeError}
												</div>
											{/if}
									...
									{:else}
										<div class="d-flex align-items-center justify-content-between gap-2">
											...
											<div class="d-flex align-items-center gap-2 flex-shrink-0">
												<span
													class="badge {similarityBadgeClass(c.score)}"
													title="Similitud: {c.score}%"
												>
													{c.score}%
												</span>
												<a
