							...
							<div class="row">
								<div class="col-md-6">
									...
									{#if peresc.genero || peresc.edad}
										<small
											>{#if peresc.genero}Género: {peresc.genero} |
											...
											{#each peresc.relaciones as relacion}
												{#each relacion.personas as personas}
													{#if personas.persona_idno != peresc.persona_idno}
														<li class="list-inline-item">
															{#if personas.polymorphic_ctype == 29}<a
					...
					{#each $searchResultsStore.groupedResults.PersonasNoEsclavizadas as pernoesc}
						<div class="list-group-item list-group-item-action">
							...
							{#if pernoesc.genero || pernoesc.edad}
								<small
									>{#if pernoesc.genero}Género: {pernoesc.genero} |
									{/if}{#if pernoesc.edad}Edad: {pernoesc.edad}{/if}</small
								><br />
							{/if}
						</div>
					{/each}
