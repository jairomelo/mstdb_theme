...
<Card {item}>
	...
	<div class="document-info mt-2">
		...
		{#if item.source.fondo || item.source.subfondo || item.source.serie || item.source.subserie}
			<p class="mb-1">
				{item.source.fondo}
				{#if item.source.subfondo}
					/ {item.source.subfondo}{/if}
				{#if item.source.serie}
					/ {item.source.serie}{/if}
				{#if item.source.subserie}
					/ {item.source.subserie}{/if}
			</p>
		{/if}
		{#if item.source.tipo_udc || item.source.unidad_documental_compuesta}
			<p class="mb-1">
				{item.source.tipo_udc}
				{#if item.source.unidad_documental_compuesta}
					/ {item.source.unidad_documental_compuesta}{/if}
			</p>
		{/if}
