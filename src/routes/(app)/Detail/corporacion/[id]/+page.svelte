...
<svelte:head>
	<title
		>{corp
			? `${corp.nombre_institucion} — Corporación — Trayectorias Afro`
			: 'Corporación — Trayectorias Afro'}</title
	>
</svelte:head>
	...
	{:else if corp}
		<div class="entity-banner">
			<h1 class="text-primary">
				<img src="/icons/i_institucion.webp" alt="Corporación" /> Corporación
			</h1>
		</div>
		<div class="card mb-4">
			<div
			...
			>
				...
				<div class="d-flex gap-2 align-items-center">
					{#if canEdit}
						<a
							href="/User/catalogar/corporacion/edit/{data.id}"
					...
					<SuggestMerge
						entity="cor"
						currentId={corp.corporacion_id}
						currentLabel={corp.nombre_institucion}
					/>
				</div>
			</div>
			<div class="card-body">
				<div class="row">
					...
					<div class="col-md-6">
						<p>
							<strong><i class="bi bi-calendar-plus me-2"></i>Creado:</strong>
							{formatDate(corp.created_at)}
						</p>
						<p>
							<strong><i class="bi bi-calendar-check me-2"></i>Actualizado:</strong>
							{formatDate(corp.updated_at)}
						</p>
					</div>
				</div>
			</div>
					...
					{#each corp.eventos as evento}
						<li class="list-group-item">
							...
							<small class="text-muted">
								Archivo: {evento.documento.archivo.nombre} ({evento.documento.archivo
									.nombre_abreviado})
							</small>
						</li>
					{/each}
			...
			<div class="card mb-4">
				...
				<ul class="list-group list-group-flush">
					{#each corp.personas_asociadas as persona}
						<li class="list-group-item">
							<a
								href="/Detail/{persona.persona_type === 'esclavizada'
									? 'personaesclavizada'
									: 'personanoesclavizada'}/{persona.persona_id}"
