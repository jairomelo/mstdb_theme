<script>
    ...
    function startEditProfile() {
        profileForm = { bio: user.profile?.bio || '', institution: user.profile?.institution || '', institution_url: user.profile?.institution_url || '' };
        profileError = null;
        profileSaved = false;
        editingProfile = true;
    }
    ...
    function startEditAccount() {
        accountForm = { username: user.username, first_name: user.first_name, last_name: user.last_name, email: user.email };
        accountError = null;
        accountSaved = false;
        editingAccount = true;
    }
    ...
    async function saveAccount() {
        ...
        try {
            ...
            setTimeout(() => { accountSaved = false; }, 3000);
        } catch (e) {
...
{#if user}
    ...
    <section class="mb-4" aria-labelledby="account-heading">
            ...
            <form on:submit|preventDefault={saveAccount} novalidate class="mt-3">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input id="acc-username" type="text" class="form-control" bind:value={accountForm.username} required maxlength="150" />
                    </div>
                    <div class="col-md-6">
                        <label for="acc-email" class="form-label">Correo electrónico</label>
                        <input id="acc-email" type="email" class="form-control" bind:value={accountForm.email} />
                    </div>
                    <div class="col-md-6">
                        <label for="acc-first" class="form-label">Nombre</label>
                        <input id="acc-first" type="text" class="form-control" bind:value={accountForm.first_name} maxlength="150" />
                    </div>
                    <div class="col-md-6">
                        <label for="acc-last" class="form-label">Apellido</label>
                        <input id="acc-last" type="text" class="form-control" bind:value={accountForm.last_name} maxlength="150" />
                    </div>
                </div>

                <div class="mt-3 d-flex gap-2 align-items-center">
                    <button type="submit" class="btn btn-primary btn-sm" disabled={accountSaving}>
                        {#if accountSaving}<span class="spinner-border spinner-border-sm me-1" aria-hidden="true"></span>{/if}
                        Guardar
                    </button>
                    <button type="button" class="btn btn-outline-secondary btn-sm" on:click={() => { editingAccount = false; }}>
                        Cancelar
                    </button>
                    {#if accountError}
        ...
        <div class="mt-3">
            {#if !showPasswordForm}
                <button class="btn btn-link btn-sm ps-0 text-secondary" on:click={() => { showPasswordForm = true; pwError = null; }}>
                ...
                <form on:submit|preventDefault={savePassword} novalidate>
                    ...
                    <div class="row g-2">
                        <div class="col-md-4">
                            ...
                            <input id="pw-current" type="password" class="form-control form-control-sm" placeholder="Contraseña actual"
                        ...
                        <div class="col-md-4">
                            ...
                            <input id="pw-new" type="password" class="form-control form-control-sm" placeholder="Nueva contraseña"
                                bind:value={pwForm.new_password} required autocomplete="new-password" minlength="8" />
                        </div>
                        <div class="col-md-4">
                            <label for="pw-confirm" class="form-label visually-hidden">Confirmar contraseña</label>
                            <input id="pw-confirm" type="password" class="form-control form-control-sm" placeholder="Confirmar contraseña"
                                bind:value={pwForm.confirm_password} required autocomplete="new-password" />
                        </div>
                    </div>
                    <div class="mt-2 d-flex gap-2 align-items-center">
                        <button type="submit" class="btn btn-primary btn-sm" disabled={pwSaving}>
                            {#if pwSaving}<span class="spinner-border spinner-border-sm me-1" aria-hidden="true"></span>{/if}
                            Cambiar
                        </button>
                        <button type="button" class="btn btn-outline-secondary btn-sm" on:click={() => { showPasswordForm = false; pwForm = { current_password: '', new_password: '', confirm_password: '' }; }}>
                            Cancelar
                        </button>
                        {#if pwError}
            ...
            <dl class="row mb-0 mt-3">
                ...
                <dd class="col-sm-8">
                    {#if user.profile?.institution_url}
                        <a href={user.profile.institution_url} target="_blank" rel="noopener noreferrer">{user.profile.institution_url}</a>
                    {:else}
                        —
                    {/if}
                </dd>
            ...
            <form on:submit|preventDefault={saveProfile} novalidate class="mt-3">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input id="pf-institution" type="text" class="form-control" bind:value={profileForm.institution} maxlength="255" />
                    </div>
                    <div class="col-md-6">
                        <label for="pf-url" class="form-label">URL de institución</label>
                        <input id="pf-url" type="url" class="form-control" bind:value={profileForm.institution_url} placeholder="https://" />
                    </div>
                    <div class="col-12">
                        <label for="pf-bio" class="form-label">Semblanza</label>
                        <textarea id="pf-bio" class="form-control" rows="4" bind:value={profileForm.bio}></textarea>
                    </div>
                </div>
                <div class="mt-3 d-flex gap-2 align-items-center">
                    <button type="submit" class="btn btn-primary btn-sm" disabled={profileSaving}>
                        {#if profileSaving}<span class="spinner-border spinner-border-sm me-1" aria-hidden="true"></span>{/if}
                        Guardar
                    </button>
                    <button type="button" class="btn btn-outline-secondary btn-sm" on:click={() => { editingProfile = false; }}>
                        Cancelar
                    </button>
                    {#if profileError}
