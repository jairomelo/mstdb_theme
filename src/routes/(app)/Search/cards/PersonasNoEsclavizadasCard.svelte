<!-- src/routes/(app)/Search/cards/PersonasNoEsclavizadasCard.svelte -->
...
<script>
	...
	const iconClass = getFilterConfigByValue(item.type)?.icon || 'bi-person-check';
	...
	let personaPersonasRelsPromise = fetchPersonaPersonasRels(
		item.source.persona_x_persona_rel || []
	);
</script>
				...
				{:then lugaresInfo}
					<ul class="list-unstyled mb-0">
						{#each lugaresInfo.slice(0, 3) as lugarRel, index}
							<li>
								<a href="/Detail/lugar/{lugarRel.lugar.lugar_id}" class="text-decoration-none">
									{lugarRel.lugar.nombre_lugar} ({lugarRel.lugar.tipo})
									{#if lugarRel.situacion_lugar}
										- {lugarRel.situacion_lugar}
									{/if}
								</a>
						...
						{#each relacionesInfo.slice(0, 3) as relacion}
							{#each relacion.personas as personas}
								{#if personas.persona_idno != item.source.persona_idno}
									<li>
										<a
											href="/Detail/{personas.polymorphic_ctype == 29
												? 'personaesclavizada'
												: 'personanoesclavizada'}/{personas.persona_id}"
											class="text-decoration-none"
										>
