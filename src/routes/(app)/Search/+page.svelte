<script>
    ...
    $: searchActive = derived(searchResultsStore, $store => $store.totalResults > 0 || $store.isLoading);
...
<div class="container mt-4">
    ...
    <div class="row mb-3">
        <div class="col">
            <div class="btn-group" role="group" aria-label="Opciones de filtro">
                <button class="btn btn-outline-primary" class:active={searchPerformed ? currentFilter === 'all' : preSelectedFilter === 'all'} on:click={() => setFilter('all')}>
                    <i class="bi bi-grid-3x3-gap me-1"></i> Todo
                </button>
                <button class="btn btn-outline-primary" class:active={searchPerformed ? currentFilter === 'documentos' : preSelectedFilter === 'documentos'} on:click={() => setFilter('documentos')}>
                    <i class="bi bi-file-text me-1"></i> Documentos
                </button>
                <button class="btn btn-outline-primary" class:active={searchPerformed ? currentFilter === 'personas_esclavizadas' : preSelectedFilter === 'personas_esclavizadas'} on:click={() => setFilter('personas_esclavizadas')}>
                    <i class="bi bi-person me-1"></i> Personas Esclavizadas
                </button>
                <button class="btn btn-outline-primary" class:active={searchPerformed ? currentFilter === 'personas_no_esclavizadas' : preSelectedFilter === 'personas_no_esclavizadas'} on:click={() => setFilter('personas_no_esclavizadas')}>
                    <i class="bi bi-person-check me-1"></i> Personas No Esclavizadas
                </button>
                <button class="btn btn-outline-primary" class:active={searchPerformed ? currentFilter === 'corporaciones' : preSelectedFilter === 'corporaciones'} on:click={() => setFilter('corporaciones')}>
                    <i class="bi bi-building me-1"></i> Corporaciones
                </button>
                <button class="btn btn-outline-primary" class:active={searchPerformed ? currentFilter === 'personas_lugar_rel' : preSelectedFilter === 'personas_lugar_rel'} on:click={() => setFilter('personas_lugar_rel')}>
    ...
    {#if !query}
        <div class="alert alert-info mt-4">
            <i class="bi bi-search me-2"></i> Ingrese un término de búsqueda para comenzar la exploración. Puede buscar documentos, personas, corporaciones o lugares relacionados con la esclavitud en Nueva España.
        </div>
    {/if}
                    ...
                    {#each $searchResultsStore.groupedResults.Documentos as doc}
                        <div class="list-group-item list-group-item-action">
                            ...
                            <p class="mb-1">
                                ...
                                {#if doc.fondo} {doc.fondo || 'Sin fondo'},{/if}
                                {#if doc.subfondo} {doc.subfondo},{/if}
                                {#if doc.serie} {doc.serie},{/if}
                                {#if doc.subserie} {doc.subserie},{/if}
                                {#if doc.tipo_udc} {doc.tipo_udc}.{/if}
                                {#if doc.unidad_documental_compuesta} {doc.unidad_documental_compuesta}.{/if}
                                {#if doc.folio_final}
                        ...
                        <div class="list-group-item list-group-item-action">
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <small>{#if peresc.genero}Género: {peresc.genero} | {/if}{#if peresc.edad}Edad: {peresc.edad}{/if}</small><br />
                                    {/if}
                                    
                                </div>
                                <div class="col-md-6">
                                    {#if peresc.documentos && peresc.documentos.length > 0}
                                        <p class="mb-1"><i class="bi bi-file-earmark-text me-2"></i>Documentos asociados:</p>
                                    ...
                                    <ul class="list-inline">
                                        {#each peresc.lugares as lugar}
                                            <li class="list-inline-item">
                                                <small>{lugar.lugar.nombre_lugar} ({lugar.lugar.tipo}) [{lugar.documento.fecha_inicial}]</small>
                                            </li>
                                        {/each}
                                    </ul>
                                </div>
                ...
                <div class="list-group mb-4">
                    {#each $searchResultsStore.groupedResults.PersonasNoEsclavizadas as pernoesc}
                        <div class="list-group-item list-group-item-action">
                            ...
                            {#if pernoesc.genero || pernoesc.edad}
                            <small>{#if pernoesc.genero }Género: {pernoesc.genero} | {/if}{#if pernoesc.edad}Edad: {pernoesc.edad}{/if}</small><br />
                            {/if}
                        </div>
                    {/each}
                </div>
    ...
    {:else}
        {#if query }
        <div class="alert alert-info mt-4">
            <i class="bi bi-info-circle me-2"></i> No se encontraron resultados para <em>{ query }</em> en { currentFilter }. Por favor, intente con otros términos o filtros.
        </div>
        {/if}
    {/if}
</div>