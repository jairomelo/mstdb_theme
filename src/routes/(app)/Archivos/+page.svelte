<script>
	...
	$: grouped = estados.map((estado) => ({
		estado,
		items: archivosData.map((a, i) => ({ ...a, _idx: i })).filter((a) => a.estado === estado)
	}));
					...
					{#each group.items as archivo}
						<div class="archivo-card mb-4">
							...
							<div class="archivo-meta">
								...
								<p class="meta-line">
									<i class="bi bi-person me-1"></i><strong>Participante:</strong>
							...
							{#if archivo.catalogos.length || archivo.informes.length || archivo.enlaces.length}
								...
								>
									...
									<i
										...
										class:bi-chevron-down={!expanded[archivo._idx]}
									></i>
								</button>
