<script>
	...
	const TIPOS_UDC = [
		{ value: 'exp', label: 'Expediente' },
		{ value: 'caj', label: 'Caja' },
		{ value: 'vol', label: 'Volumen' },
		{ value: 'lib', label: 'Libro' },
	...
	async function saveNewArchivo() {
		...
		try {
			const payload = {
				...
				...(newArchivo.nombre_abreviado.trim() && {
					nombre_abreviado: newArchivo.nombre_abreviado.trim()
				}),
				...(newArchivo.ubicacion_archivo && {
					ubicacion_archivo: newArchivo.ubicacion_archivo.value
				})
			};
			const result = await createArchivo(payload);
			archivo = {
				value: result.archivo_id,
				label: `[${result.nombre_abreviado}] ${result.nombre}`
			};
	...
	function reset() {
		...
		deteriorado = false;
		fecha_inicial = '';
		fecha_inicial_aproximada = false;
		fecha_final = '';
		fecha_final_aproximada = false;
		lugar_de_produccion = null;
		folio_inicial = '';
		folio_final = '';
		evento_valor_sp = '';
...
<div class="container mt-4">
	...
	{#if created}
		<div class="alert alert-success d-flex align-items-center justify-content-between" role="alert">
			<span
				>Documento creado: <strong>{created.short_id ?? created.documento_idno}</strong> — {created.titulo ??
					titulo}</span
			>
	...
	<form on:submit|preventDefault={handleSubmit} novalidate>
		<section class="card mb-4">
			...
			<div class="card-body row g-3">
				<div class="col-12">
					...
					{#if showNewArchivo}
						<div class="border rounded p-3 mt-2 bg-light">
							...
							<div class="row g-2">
								<div class="col-12">
									<label for="na-nombre" class="form-label form-label-sm"
								...
								<div class="col-md-8">
									...
									<SearchableSelect
										id="na-lugar"
										bind:value={newArchivo.ubicacion_archivo}
										endpoint="lugares/"
							...
							<div class="mt-3">
								<button
									...
									on:click={saveNewArchivo}
									disabled={newArchivoSaving}
								>
									{#if newArchivoSaving}<span
				...
				<div class="col-md-6">
					<FormField label="Fondo" id="fondo" required error={fieldError('fondo')}>
						<input
							id="fondo"
							class="form-control"
							class:is-invalid={fieldError('fondo')}
							bind:value={fondo}
							required
						/>
					</FormField>
				...
				<div class="col-md-6">
					<FormField
						label="Unidad documental compuesta"
						id="udc"
						required
						error={fieldError('unidad_documental_compuesta')}
					>
						<input
							id="udc"
							class="form-control"
							class:is-invalid={fieldError('unidad_documental_compuesta')}
							bind:value={unidad_documental_compuesta}
							required
						/>
					</FormField>
				</div>
			...
			<div class="card-body row g-3">
				...
				<div class="col-12">
					<FormField label="Título" id="titulo" required error={fieldError('titulo')}>
						<input
							id="titulo"
							class="form-control"
							class:is-invalid={fieldError('titulo')}
							bind:value={titulo}
							required
						/>
					</FormField>
				...
				<div class="col-12">
					<FormField label="Descripción" id="descripcion" error={fieldError('descripcion')}>
						<textarea id="descripcion" class="form-control" rows="3" bind:value={descripcion}
						></textarea>
					</FormField>
				</div>
				<div class="col-12">
			...
			<div class="card-body row g-3">
				...
				<div class="col-md-2 d-flex align-items-end pb-3">
					<div class="form-check">
						<input
							...
							type="checkbox"
							id="fi-aprox"
							bind:checked={fecha_inicial_aproximada}
						/>
				...
				<div class="col-md-2 d-flex align-items-end pb-3">
					<div class="form-check">
						<input
							...
							type="checkbox"
							id="ff-aprox"
							bind:checked={fecha_final_aproximada}
						/>
				...
				<div class="col-md-10">
					<FormField label="Lugar de producción" id="ldp" error={fieldError('lugar_de_produccion')}>
						<SearchableSelect
							id="ldp"
							bind:value={lugar_de_produccion}
							endpoint="lugares/"
							placeholder="Buscar lugar…"
						/>
					</FormField>
			...
			<div class="card-body row g-3">
				<div class="col-md-4">
					<FormField
						...
						id="folio-ini"
						required
						error={fieldError('folio_inicial')}
					>
						<input
							id="folio-ini"
							class="form-control"
							class:is-invalid={fieldError('folio_inicial')}
							bind:value={folio_inicial}
							required
						/>
					</FormField>
				</div>
