<script>
	...
	async function loadAll() {
		...
		try {
			const [docData, peData, pneData, corpData] = await Promise.all([
				...
				fetchWithBaseUrl(
					`personas-no-esclavizadas/?documentos__documento_id=${docId}&page_size=100`
				),
	...
	async function reloadSection(section) {
		...
		const data = await fetchWithBaseUrl(
			`${endpoint}/?documentos__documento_id=${docId}&page_size=100`
		);
			...
			<div class="card-header d-flex justify-content-between align-items-start gap-2">
				<div>
					<h5 class="mb-0 fw-semibold lh-sm">
						{doc.titulo}
						{#if doc.deteriorado}
							<span class="badge bg-warning text-dark ms-2 align-middle" style="font-size:.7rem"
								>Deteriorado</span
							>
						{/if}
					</h5>
				...
				<div class="d-flex gap-2 flex-shrink-0">
					{#if $hasPerm('dbgestor.change_documento')}
						<button
							...
							aria-controls="metaEditPanel"
						>
							<i class="bi {editMode ? 'bi-x-lg' : 'bi-pencil'} me-1" aria-hidden="true"
							></i>{editMode ? 'Cerrar' : 'Editar'}
						</button>
					{/if}
					{#if $hasPerm('dbgestor.delete_documento')}
						<button
							class="btn btn-sm btn-outline-danger"
							on:click={() => (confirmDocDelete = true)}
						>
			...
			<div class="card-body pb-2">
				<dl class="row g-0 mb-0">
					...
					{#if doc.fondo}
						...
						<dd class="col-sm-9 small py-1 border-bottom mb-0">
							{doc.fondo}{doc.subfondo ? ` / ${doc.subfondo}` : ''}{doc.serie
								? ` / ${doc.serie}`
								: ''}{doc.subserie ? ` / ${doc.subserie}` : ''}
						</dd>
					{/if}
					{#if doc.unidad_documental_compuesta}
						<dt class="col-sm-3 text-muted fw-normal small py-1 border-bottom">Unidad doc.</dt>
						<dd class="col-sm-9 small py-1 border-bottom mb-0">
							{TIPO_UDC_LABELS[doc.tipo_udc] ?? doc.tipo_udc}
							{doc.unidad_documental_compuesta}
							{#if doc.sigla_documento}<span class="text-muted ms-2">· {doc.sigla_documento}</span
								>{/if}
						</dd>
					{/if}
					...
					{#if doc.lugar_de_produccion}
						<dt class="col-sm-3 text-muted fw-normal small py-1 border-bottom">
							Lugar de producción
						</dt>
						<dd class="col-sm-9 small py-1 border-bottom mb-0">
							{doc.lugar_de_produccion.nombre_lugar}
						</dd>
					{/if}
					{#if doc.descripcion}
					...
					{#if doc.notas}
						...
						<dd class="col-sm-9 small py-1 border-bottom mb-0" style="white-space:pre-line">
							{doc.notas}
						</dd>
					{/if}
			...
			<div class="card mb-4" id="metaEditPanel">
				<div class="card-header cataloguer-section-header py-2">
					<span class="fw-semibold"
						><i class="bi bi-pencil-square me-1" aria-hidden="true"></i>Editar datos del documento</span
					>
					{#if $hasPerm('dbgestor.change_documento')}
						<button
						...
						>
							{#if metaSaving}<span class="spinner-border spinner-border-sm me-1" aria-hidden="true"
								></span>{/if}
							Guardar cambios
						</button>
					{/if}
									...
									<div class="border rounded p-2 mt-2 bg-light">
										...
										<div class="d-flex gap-2">
											<input
												...
												placeholder="Nombre del tipo (ej. Escritura de venta)"
											...
											<button
												...
												on:click={async () => {
													...
													newTipoError = null;
													try {
														const result = await createTipoDocumental({
															tipo_documental: newTipoNombre.trim()
														});
						...
						<div class="col-md-6">
							<FormField label="Archivo" error={metaErrors.archivo?.[0]}>
								<SearchableSelect
									...
									value={doc.archivo
										? {
												value: doc.archivo.archivo_id,
												label: `[${doc.archivo.nombre_abreviado ?? doc.archivo.archivo_idno}] ${doc.archivo.nombre}`
											}
										: null}
									on:change={(e) => {
										metaForm.archivo = e.detail?.value ?? null;
										metaDirty = true;
									}}
								/>
							</FormField>
		...
		<div class="d-flex flex-column gap-3">
			{#if activeSection === 'pe'}
				<FormField label="Buscar persona esclavizada" error={slideErrors.detail?.[0]}>
					<SearchableSelect
						...
						searchParam="search"
						placeholder="Nombre, apellidos, idno..."
			...
			<div class="d-flex gap-2 justify-content-end mt-2">
				...
				<button
					...
					disabled={slideSaving || !linkTarget}
		...
		<div class="d-flex flex-column gap-3">
			{#if activeSection === 'pe'}
				<FormField label="Nombres *" error={slideErrors.nombres?.[0]}>
					<input
						class="form-control"
						class:is-invalid={slideErrors.nombres}
						bind:value={newPE.nombres}
					/>
				</FormField>
			...
			{:else if activeSection === 'inst'}
				<FormField label="Nombre institución *" error={slideErrors.nombre_institucion?.[0]}>
					<input
						class="form-control"
						class:is-invalid={slideErrors.nombre_institucion}
				...
				<FormField label="Tipo de institución *" error={slideErrors.tipo_institucion?.[0]}>
					<SearchableSelect
						...
						bind:value={newInst.tipo_institucion}
					/>
				</FormField>
			{/if}
