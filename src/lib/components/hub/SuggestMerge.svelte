...
<button type="button" class="btn btn-sm btn-outline-secondary" on:click={openModal}
        title="Sugerir fusión de registros duplicados">
    <i class="bi bi-git me-1"></i>Sugerir merge
</button>

{#if open}
    <div class="modal d-block" tabindex="-1" role="dialog" aria-modal="true" aria-labelledby="suggest-merge-title"
                ...
                <div class="modal-body">
                    {#if success}
                        <div class="alert alert-success" role="alert">
                            <i class="bi bi-check-circle-fill me-2"></i>Sugerencia enviada. El equipo de staff la revisará.
                        </div>
                        <button class="btn btn-outline-secondary" on:click={closeModal}>Cerrar</button>
                    {:else}
                        <p class="text-muted small mb-3">
                            Indica cuál es el registro canónico (a conservar) y cuál es el duplicado (a fusionar y eliminar).
                            El equipo de staff revisará la sugerencia antes de ejecutar la fusión.
                        </p>
                        ...
                        <div class="mb-3">
                            ...
                            <input id="suggest-canonical" type="number" class="form-control"
                                   bind:value={canonicalId} placeholder="ID del registro definitivo" required />
                            {#if currentId}
                                <div class="form-text">Registro actual: <strong>{currentLabel}</strong> (ID {currentId})</div>
                            {/if}
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input id="suggest-duplicate" type="number" class="form-control"
                                   bind:value={duplicateId} placeholder="ID del registro a fusionar" required />
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-semibold" for="suggest-notas">Notas (opcional)</label>
                            <textarea id="suggest-notas" class="form-control" rows="2"
                                      bind:value={notas} placeholder="Por qué crees que son el mismo registro…"></textarea>
                        </div>
