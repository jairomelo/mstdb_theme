...
export const columnsPersonasEsclavizadas = [
	...
	{ data: 'sexo', title: 'Sexo' },
	{
		...
		render: (data, type, row) =>
			data
				? `${data} ${row.unidad_temporal_edad === undefined ? 'años' : row.unidad_temporal_edad}`
				: '-',
		title: 'Edad'
	},
	{
		...
		render: (data, type, row) => {
			...
			return data
				.map((relacion) => {
					...
					const otherPersonas = relacion.personas.filter(
						(persona) => persona.persona_id !== row.persona_id
					);
					...
					return otherPersonas
                        ...
                        data-bs-title="${relacion.descripcion_relacion || ''}">${
												...
												} (${relacion.naturaleza_relacion})</a>`;
						})
						.join(', ');
				})
				.filter(Boolean)
	...
	},
	{
		...
		render: (data) => {
			...
			return sortedLugares
				.map((item) => {
					const fecha = new Date(item.documento.fecha_inicial).getFullYear();
