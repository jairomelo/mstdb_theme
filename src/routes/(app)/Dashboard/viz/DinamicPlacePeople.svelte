<script>
	...
	const processData = (rawData) => {
		...
		const filteredData =
			selectedPlace === 'all' ? rawData : rawData.filter((d) => d.lugar === selectedPlace);
	...
	const renderPlot = (processedData) => {
		...
		const layout = {
			...
			xaxis: {
				...
				range: [
					Math.min(...processedData.map((d) => d.year)) - 2,
			...
			yaxis: {
				...
				range: [
					-0.5,
					Math.max(...processedData.flatMap((d) => d.data.map((item) => item.originalY))) * 1.1
				],
				tickformat: 'd'
			},
			updatemenus: [
		...
		Plotly.newPlot(plotDiv, frames[0].data, layout, config).then(() =>
		...
		const summaryData = uniquePlaces
			.map((place) => {
