    ...
    {:else if peresc}
        ...
        {#if peresc.documentos && peresc.documentos.length > 0}
            <div class="card mb-4">
                <div class="card-header bg-secondary text-white">
                    <h2 class="card-title h5 mb-0"><i class="bi bi-file-earmark-text me-2"></i>Documentos relacionados</h2>
                </div>
                ...
                <ul class="list-group list-group-flush">
                    {#each peresc.lugares as lugar}
                        <li class="list-group-item">
                            ...
                            {#if lugar.situacion_lugar}<p class="mb-1"><strong>Situación:</strong> {lugar.situacion_lugar}</p>{/if}
                        </li>
                    {/each}
                </ul>
            </div>
        ...
        <div class="mt-3 text-muted">
            <small data-bs-toggle="tooltip" data-bs-placement="top" title="Fecha de creación">
                <i class="bi bi-clock-history me-1"></i>Creado: {new Date(peresc.created_at).toLocaleString()}
            </small>
