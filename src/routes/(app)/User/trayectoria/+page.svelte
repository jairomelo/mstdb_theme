<script>
	...
	async function selectPersona(pe) {
		...
		try {
			selectedPersona = await peresclavizadas(pe.persona_id);
		} catch {
			selectedPersona = pe;
		}
	...
	async function initMap() {
		...
		if (validPoints.length > 1) {
			map.fitBounds(
				L.latLngBounds(validPoints.map((p) => [parseFloat(p.lugar.lat), parseFloat(p.lugar.lon)])),
				{ padding: [30, 30] }
			);
		}
	}
	...
	function toggleSwap(point) {
		...
		if (!swapA) {
			...
			if (swapA === point.persona_x_lugares) {
				swapA = null;
				return;
			}
	...
	function openEditPoint(point) {
		...
		editSituacion =
			typeof point.situacion_lugar === 'object'
	...
	function onAddLugarInput() {
		...
		if (addLugarQuery.trim().length < 2) {
			addLugarResults = [];
			return;
		}
				...
				<div class="card h-100">
					<div class="card-header d-flex justify-content-between align-items-center">
						<span class="fw-semibold">
							...
							<Tooltip
								text="Cada punto representa un lugar donde apareció esta persona en los documentos. Los puntos con 🔗 vienen de campos del perfil (nacimiento, procedencia, defunción) y no se pueden editar aquí."
							/>
						</span>
						<div class="d-flex gap-2 align-items-center">
							{#if swapA}
								...
								<button class="btn btn-sm btn-outline-secondary" on:click={() => (swapA = null)}
									>Cancelar</button
								>
							{/if}
							<button
									...
									<div class="d-flex align-items-start gap-2 traj-point-row">
										...
										<div class="flex-grow-1 min-width-0 traj-point-content">
											...
											<small class="text-muted d-block">
												{point.situacion_lugar ?? '—'}
												{#if point.fecha_inicial_lugar}
													· {point.fecha_inicial_lugar}{/if}
											</small>
											{#if isFk}
												<small class="text-muted d-block fst-italic"
													>Campo del registro de persona</small
												>
											{:else if point.documento}
												<small
													class="text-muted d-flex align-items-start gap-1 traj-documento-line"
												>
													<i class="bi bi-file-text"></i>
													<span class="traj-documento-title" title={documentoLabel}
														>{documentoLabel}</span
													>
												</small>
											{/if}
										...
										<div class="d-flex flex-column gap-1 traj-point-actions">
											{#if isReadOnly}
												<span
													...
													title={isFk
														? 'Campo del registro de persona'
														: 'Punto de evento (no intercambiable)'}
												>
											...
											{#if isFk}
												<button
													...
													title="Agregar como punto de trayectoria"
													on:click={() => openAddPanelWithLugar(point)}
												>
													<i class="bi bi-plus-circle"></i>
												</button>
												<a
													href="/Update/personaesclavizada/{selectedPersona.persona_id}"
													target="_blank"
												...
												<button
													...
													title="Editar coordenadas del lugar"
													on:click={() => openEditCoords(point)}
												>
													<i class="bi bi-geo"></i>
												</button>
											{:else}
												<button
													class="btn btn-sm btn-outline-primary"
													title="Editar fecha y situación"
													on:click={() => openEditPoint(point)}
												>
	...
	<div class="slideover" role="dialog" aria-modal="true" aria-label="Editar coordenadas">
		...
		<div class="slideover-body">
			<div class="alert alert-warning" role="alert">
				...
				<strong>Atención:</strong> <em>{editingCoords.nombre_lugar}</em> es un lugar compartido. Los
				cambios afectarán a <strong>todas las personas</strong> que tengan este lugar en su trayectoria.
			</div>
			<div class="mb-3">
...
{#if addPanelOpen}
	...
	<div class="slideover" role="dialog" aria-modal="true" aria-label="Añadir punto">
		...
		<div class="slideover-body">
			<!-- Documento selector -->
			<div class="mb-3">
				<label class="form-label" for="add-documento"
			...
			<div class="mb-3">
				...
				<input
					...
					type="number"
					class="form-control"
					bind:value={addOrdinal}
					placeholder="e.g. 1, 2, -1…"
				/>
				<small class="form-text text-muted d-block">
					El 0 está reservado para el lugar del evento/transacción. Usa negativos para lugares
					previos al evento y positivos para los posteriores.
				</small>
			</div>
			<!-- Situacion -->
			<div class="mb-3">
		...
		<div class="slideover-footer">
			...
			<button
				...
				disabled={addSaving || !addSelectedLugar}
				on:click={saveAddPoint}
			>
	...
	<div class="modal-dialog-custom" role="dialog" aria-modal="true">
		<div class="modal-content">
			...
			<div class="modal-body">
				<p>
					¿Eliminar <strong>{deletingPoint.lugar?.nombre_lugar}</strong> (ordinal {deletingPoint.ordinal})
					de la trayectoria?
				</p>
